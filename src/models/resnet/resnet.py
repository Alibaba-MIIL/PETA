...
__all__ = ['resnet50']  # model_registry will add each entrypoint fn to this
...
# dont use this pretrain urls. train yourself !
default_cfgs = {
...
class FlattenDropout(nn.Module):
    def __init__(self, drop_rate, training):
        super(FlattenDropout, self).__init__()
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3, use_se=False,
        ...
        if deep_stem:
            ...
            conv1 = nn.Conv2d(in_chans, stem_width, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        layer1 = self._make_layer(block, 64, layers[0], stride=1, **largs)  # downsample on enter
        layer2 = self._make_layer(block, 128, layers[1], stride=2, **largs)
        layer3 = self._make_layer(block, 256, layers[2], stride=stride_3_4, dilation=dilation_3, **largs)
        layer4 = self._make_layer(block, 512, layers[3], stride=stride_3_4, dilation=dilation_4, **largs)
        ...
        global_pool_layer = SelectAdaptivePool2d(pool_type=global_pool)
        flatten_dropout = FlattenDropout(self.drop_rate, self.training)
        fc = nn.Linear(self.num_features * global_pool_layer.feat_mult(), num_classes)
        self.head = nn.Sequential(OrderedDict([
            ('global_pool', global_pool_layer),
            ('FlattenDropout', flatten_dropout),
            ('fc', fc)]))
...
@register_model
def resnet101d(model_params, in_chans=3, **kwargs):
    """Constructs a ResNet-101-D model.
    """
    num_classes = model_params['num_classes']
    model = ResNet(
        Bottleneck, [3, 4, 23, 3], stem_width=32, deep_stem=True, avg_down=True,
        num_classes=num_classes, in_chans=in_chans, **kwargs)
    return model

@register_model
def resnet101(model_params, in_chans=3, **kwargs):
