...
def load_classes(classes_path, args):
  with open(classes_path, 'rb') as fp:
    classes, class_to_idx, idx_to_class = pickle.load(fp)
  args.classes = classes
  args.class_to_idx = class_to_idx
  args.idx_to_class = idx_to_class
  return args


def create_val_dataset(args, transform):
...
def create_dataloader(args):
  ...
  '''
  args = load_classes(args.classes_path, args=args)
...
def validate(model, val_loader):
  ...
  accum = AccumMetricG(None, dim_argmax=None, sigmoid=True, thresh=None, to_np=False,
